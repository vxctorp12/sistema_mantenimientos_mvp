...
<head>
    ...
    <style>
        ...
        tr.subbanner-blue td {
            background-color: #1e3a8a !important; /* Azul Rey Oscuro */
            color: #ffffff !important;
            font-weight: 700;
            font-size: 8.5px;
            text-transform: uppercase;
        ...
        tr.subbanner-gray td {
            background-color: #334155 !important; /* Gris Pizarra Oscuro Alto Contraste */
            color: #ffffff !important;
            font-weight: 700;
            font-size: 8.5px;
            text-transform: uppercase;
...
<div class="container">
        ...
        <tbody>
            <tr>
                <td style="width: 45%;">
                    ...
                    <span class="checkbox-box">{{ in_array(strtoupper($mantenimiento->equipo->tipo_equipo ?? ''), ['DESKTOP', 'ESCRITORIO']) ? 'X' : '' }}</span> Desktop &nbsp;&nbsp;&nbsp;
                    <span class="checkbox-box">{{ strtoupper($mantenimiento->equipo->tipo_equipo ?? '') === 'LAPTOP' ? 'X' : '' }}</span> Laptop
                </td>
    ...
    <!-- CHECK LIST PARA LOS MANTENIMIENTOS DE EQUIPOS -->
    <table class="data-table">
        <thead>
            <tr class="banner-main">
                <th colspan="3" class="banner-main">CHECK LIST PARA LOS MANTENIMIENTO DE EQUIPOS</th>
            </tr>
            <tr class="subbanner-blue">
        ...
        <tbody>
            <tr>
                <td><span class="circle-icon">¡</span> Verificar condiciones físicas: manchas, rayones, fisuras, golpes, cables reventados y estado general del equipo.</td>
                <td colspan="2" class="text-center"><span class="checkbox-box">X</span></td>
            </tr>
            <tr>
                <td><span class="circle-icon">¡</span> Encender el equipo y comprobar que el hardware y el sistema operativo funcionan correctamente.</td>
                <td colspan="2" class="text-center"><span class="checkbox-box">X</span></td>
            </tr>
            <tr>
                <td><span class="circle-icon">¡</span> Registrar cualquier condición previa o componente dañado antes de iniciar el mantenimiento.</td>
                <td colspan="2" class="text-center"><span class="checkbox-box">X</span></td>
            </tr>
            <tr class="subbanner-blue">
            ...
            <tr>
                <td colspan="3" style="height: 16px;">
                    {{ $mantenimiento->diagnostico ?? $mantenimiento->recomendaciones ?? 'Equipo ingresa para mantenimiento preventivo periódico programado.' }}
                </td>
            </tr>
        </tbody>
    </table>

    <!-- MATRIZ DE LIMPIEZA EXTERNA E INTERNA -->
    @php
        ...
        $limpiezaExterna = [
            ...
            'TECLADO',
            'MOUSE / MOUSEPAD',
            'CABLES Y CONECTORES DE ALIMENTACIÓN',
            'CASE / PUERTOS / LECTORES',
    ...
    <table class="data-table">
        ...
        <tbody>
            @foreach($limpiezaExterna as $row)
                @php
                    $match = null;
            ...
            @endforeach
            <tr class="subbanner-gray">
                <th class="subbanner-gray" style="text-align: left;">LIMPIEZA INTERNA</th>
            ...
            @foreach($limpiezaInterna as $row)
                @php
                    $match = null;
                    foreach ($itemsMap as $k => $v) {
                        if (str_contains($k, explode(' ', $row)[0])) { $match = $v; break; }
    ...
    <table class="data-table">
        ...
        <tbody>
            <tr>
                <td><span class="circle-icon">¡</span> Encender el equipo y verificar funcionamiento de: hardware, sistema operativo, conectividad y ajustes visuales de monitor.</td>
            ...
            <tr>
                <td colspan="2" style="height: 24px; vertical-align: top;">
                    {{ $mantenimiento->observaciones ?? $mantenimiento->trabajo_realizado ?? 'Mantenimiento preventivo ejecutado satisfactoriamente. Equipo queda en perfecto estado operativo.' }}
                </td>
            </tr>
        </tbody>
    </table>
